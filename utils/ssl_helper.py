...
def configure_ssl_verification(disable_ssl: Optional[bool] = None) -> bool:
    ...
    if disable_ssl is None:
        disable_ssl = os.environ.get('DISABLE_TELEGRAM_SSL_VERIFY', 'False').lower() in ('true', '1', 't', 'yes')
    ...
    if disable_ssl:
        ...
        # 添加警告信息，标记为临时解决方案
        logger.warning("⚠️ SSL 证书验证已禁用。这是临时解决方案，请考虑以下安全替代方案：")
        logger.warning("1. 更新系统证书库")
        logger.warning("2. 检查代理配置是否正确")
        logger.warning("3. 如使用自建代理，确保配置正确的 SSL 传递")
        
        logger.warning("⚠️ SSL 证书验证已完全禁用。这可能存在安全风险，仅建议在特殊网络环境或开发环境中使用。")
        return True
    else:
        # 确保环境变量设置正确
