...
def test_all_configurations() -> Dict:
    ...
    if https_proxy:
        configs.append(
            {"name": "HTTPS 代理 + SSL 验证", "proxy": https_proxy, "verify_ssl": True}
        )

    # 2. HTTP 代理 + 禁用 SSL
    if https_proxy:
        configs.append(
            {"name": "HTTPS 代理 + 禁用 SSL", "proxy": https_proxy, "verify_ssl": False}
        )

    # 3. SOCKS 代理 + SSL 验证
    if socks_proxy and "socks" in socks_proxy.lower():
        configs.append(
            {"name": "SOCKS 代理 + SSL 验证", "proxy": socks_proxy, "verify_ssl": True}
        )

    # 4. SOCKS 代理 + 禁用 SSL
    if socks_proxy and "socks" in socks_proxy.lower():
        configs.append(
            {"name": "SOCKS 代理 + 禁用 SSL", "proxy": socks_proxy, "verify_ssl": False}
        )

    # 5. 直接连接 + SSL 验证
    configs.append({"name": "直接连接 + SSL 验证", "proxy": None, "verify_ssl": True})
    ...
    if results:
        ...
        logger.info(
            f"最佳配置：{best_config['name']}, 耗时：{min(results, key=lambda x: x[1])[1]:.2f}秒"
        )
